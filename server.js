...
app.post('/update_menu_item', async (req, res) => {
  try {
    ...
    // Update the MenuItem collection
    const menuItem = await MenuItem.findOne({ name });
    if (!menuItem) {
      return res.status(404).send('Menu item not found in MenuItems');
    }

    // Update fields in MenuItem
    menuItem.type = type;
    menuItem.description = description;
    menuItem.price = parseFloat(price);
    await menuItem.save();

    // Update or create the corresponding entry in the Dish collection
    ...
    if (!dish) {
      ...
      const newDish = new Dish({
        ...
        image_path, // Save the image path
      });
      await newDish.save();
    } else {
      // If the dish exists, update the relevant fields
      dish.type = type;
      dish.description = description;
      dish.price = parseFloat(price);
      dish.image_path = image_path; // Ensure this is updated in Dishes
      await dish.save();
    }

    res.status(200).send('Menu item and Dish updated successfully');
  } catch (error) {
    res.status(500).send('Error updating menu item and dish');
  }
});
...
app.get("/api/popular_items", async (req, res) => {
  try {
    const popularItems = await PopularItem.find(); // Fetch all popular items
    res.json(popularItems); // Return popular items in JSON format
  } catch (err) {
...
app.post('/orders/update-order', async (req, res) => {
    const { member_id, items , remarks , orderType} = req.body;
